...
const repositories = { hafleet: 'hagency-org/HAFleet', robrix2: 'Project-Robius-China/robrix2', palpo: 'palpo-im/palpo' };
const results = await Promise.all(Object.entries(repositories).map(async ([id, repo]) => {
  ...
  const assets = release.assets.filter(asset => /\.(tar\.gz|zip|apk|deb|dmg|exe)$/.test(asset.name)).map(asset => {
    const name = asset.name.toLowerCase();
    const platform = id === 'hafleet' ? 'macos,linux' : /macos|apple-darwin/.test(name) ? 'macos' : /windows/.test(name) ? 'windows' : /android/.test(name) ? 'android' : /linux/.test(name) ? 'linux' : null;
...
await writeFile(new URL('../src/data/releases.json', import.meta.url), JSON.stringify({ verifiedAt: new Date().toISOString(), releases: results }, null, 2) + '\n');
console.log(`Verified ${results.length} releases and ${results.reduce((n, r) => n + r.assets.length, 0)} downloadable assets. Review content version labels before committing.`);
